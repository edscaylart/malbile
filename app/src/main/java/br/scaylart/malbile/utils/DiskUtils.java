...
public class DiskUtils {
    ...
    public static String[] getStorageDirectories() {
        ...
        storageDirectories.add(MalbileApplication.getInstance().getFilesDir().getAbsolutePath());

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            File[] directories = MalbileApplication.getInstance().getExternalFilesDirs(null);
            if (directories != null) {
            ...
            if (TextUtils.isEmpty(rawEmulatedStorageTarget)) {
                if (TextUtils.isEmpty(rawExternalStorage)) {
                    storageDirectories.add("/storage/sdcard0" + File.separator + MalbileApplication.getInstance().getPackageName());
                } else {
                    storageDirectories.add(rawExternalStorage + File.separator + MalbileApplication.getInstance().getPackageName());
                }
            } else {
                ...
                if (TextUtils.isEmpty(rawUserId)) {
                    storageDirectories.add(rawEmulatedStorageTarget + File.separator + MalbileApplication.getInstance().getPackageName());
                } else {
                    storageDirectories.add(rawEmulatedStorageTarget + File.separator + rawUserId + File.separator + MalbileApplication.getInstance().getPackageName());
                }
            }
            ...
            if (!TextUtils.isEmpty(rawSecondaryStoragesStr)) {
                ...
                for (int index  = 0; index < rawSecondaryStorages.length; index++) {
                    storageDirectories.add(rawSecondaryStorages[index] + File.separator + MalbileApplication.getInstance().getPackageName());
                }
            }
        }
